...
原子性、一致性、持久性通过数据库的redo log和undo log 来完成的。redo log成为重做日志，是用来保证事物原子性和持久性的。undo log 用来保证事物的一致性。

redo 恢复提交事物修改的页操作，undo 回归记录到某个特定版本。redo通常是物理日志，记录的是页的物理修改操作。undo 是逻辑日志，根据每行记录进行记录。
